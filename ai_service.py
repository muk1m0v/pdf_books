...
async def ask_about_book(question: str, book_title: str, book_text: str) -> str:
    ...
    prompt = (
        ...
        f"Форматируй ответ Markdown-разметкой Telegram: *жирный текст*, _курсив_, "
        f"`код` — только эти три вида разметки, без заголовков (#), без таблиц, "
        f"без вложенных списков со звёздочками в начале строки.\n\n"
        f"ТЕКСТ КНИГИ:\n{context}\n\n"
        f"ВОПРОС: {question}"
    )
